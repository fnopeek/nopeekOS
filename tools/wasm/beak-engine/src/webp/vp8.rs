...
use super::{ByteReader, DecodingError, SliceReader};
...
impl BoolReader {
    ...
    pub(crate) fn init(&mut self, buf: Vec<u8>) -> Result<(), DecodingError> {
        ...
        self.reader = ByteReader::new(buf);
        self.value = self.reader.read_u16_be()? as u32;
        self.range = 255;
        self.bit_count = 0;
        ...
        if self.range < 128 {
            ...
            if self.bit_count >= 8 {
                ...
                match self.reader.read_u8() {
                    ...
                    Err(_) if !self.eof => self.eof = true,
    ...
    pub(crate) fn fill_bgra(&self, buf: &mut [u8]) {
        ...
        for (y, row) in buf.chunks_exact_mut(usize::from(self.width) * 4).enumerate() {
            ...
            for (x, px) in row.chunks_exact_mut(4).enumerate() {
                let chroma_index = chroma_index_row + x / 2;
                let (yy, u, v) =
                    (self.ybuf[index], self.ubuf[chroma_index], self.vbuf[chroma_index]);
                px[0] = clip(mulhi(yy, 19077) + mulhi(u, 33050) - 17685);
                px[1] = clip(mulhi(yy, 19077) - mulhi(u, 6419) - mulhi(v, 13320) + 8708);
                px[2] = clip(mulhi(yy, 19077) + mulhi(v, 26149) - 14234);
                px[3] = 255;
                index += 1;
            }
        }
...
impl<'a> Vp8Decoder<'a> {
    ...
    pub fn new(r: SliceReader<'a>) -> Vp8Decoder<'a> {
        let f = Frame::default();
    ...
    fn init_partitions(&mut self, n: usize) -> Result<(), DecodingError> {
        if n > 1 {
            ...
            for (i, s) in sizes.chunks(3).enumerate() {
                ...
                let size = u32::from_le_bytes([s[0], s[1], s[2], 0]);

...
mod tests {
    ...
    fn transient_is_detected() {
        ...
        for (i, x) in s.iter_mut().enumerate() {
            *x = (0.02 * (2.0 * core::f64::consts::PI * 400.0 * i as f64 / sr).sin()) as f32;
        }
        // A loud burst at the start of frame 2.
        for i in 0..600 {
            let env = (1.0 - i as f64 / 600.0).max(0.0);
            s[2 * FRAME_LEN + i] +=
                (0.8 * env * (2.0 * core::f64::consts::PI * 3000.0 * i as f64 / sr).sin()) as f32;
        }
    ...
    fn transient_encodes_short_and_decodes() {
        ...
        for i in 0..n {
            ...
            let mut v = 0.02 * (2.0 * core::f64::consts::PI * 400.0 * t).sin();
            if (2 * FRAME_LEN..2 * FRAME_LEN + 600).contains(&i) {
                let k = i - 2 * FRAME_LEN;
                let env = (1.0 - k as f64 / 600.0).max(0.0);
                v += 0.8 * env * (2.0 * core::f64::consts::PI * 3000.0 * k as f64 / sr as f64).sin();
            }
    ...
    fn stereo_ms_roundtrips_mono_content() {
        ...
        for i in 0..n {
            let s =
                (0.4 * (2.0 * core::f64::consts::PI * 600.0 * i as f64 / sr as f64).sin()) as f32;
    ...
    fn stereo_direct_decode_amplitude() {
        ...
        for i in 0..n {
            let t = i as f64 / sr as f64;
            let l = (0.4 * (2.0 * core::f64::consts::PI * 440.0 * t).sin()) as f32;
            let r = (0.4 * (2.0 * core::f64::consts::PI * 660.0 * t).sin()) as f32;
            interleaved.push(l);
            interleaved.push(r);
        }
        let mut enc = AacEncoder::default();
    ...
    fn profile_encode_hotpath() {
        ...
        for (i, s) in cur.iter_mut().enumerate() {
            let t = i as f64 / sr as f64;
            *s = (0.3 * (2.0 * core::f64::consts::PI * 440.0 * t).sin()
                + 0.1 * (2.0 * core::f64::consts::PI * 3000.0 * t).sin()) as f32;
        }
    ...
    fn tone_energy(sig: &[f32], freq: f64, sr: f64) -> f64 {
        ...
        for (i, &s) in sig.iter().enumerate() {
            let ph = 2.0 * core::f64::consts::PI * freq * i as f64 / sr;
            re += s as f64 * ph.cos();
            im += s as f64 * ph.sin();
        }
    ...
    fn encodes_and_decodes_recognizable_tone() {
        ...
        for i in 0..n {
            let s =
                ((i as f64 * 2.0 * core::f64::consts::PI * 440.0 / sr as f64).sin() * 0.5) as f32;
            samples.push(s);
        }
    ...
    fn rate_loop_respects_bitrate() {
        ...
        for i in 0..n {
            ...
            let mut s = 0.0;
            for h in 1..=8 {
                s += (2.0 * core::f64::consts::PI * 300.0 * h as f64 * t).sin() / h as f64;
            }
            st ^= st << 13;
            st ^= st >> 17;
            st ^= st << 5;
    ...
    fn psy_model_shapes_noise_below_flat() {
        ...
        for (i, s) in cur.iter_mut().enumerate() {
            let t = i as f64 / sr as f64;
            let v = 0.5 * (2.0 * core::f64::consts::PI * 500.0 * t).sin()
                + 0.25 * (2.0 * core::f64::consts::PI * 1500.0 * t).sin()
                + 0.15 * (2.0 * core::f64::consts::PI * 4000.0 * t).sin();
            *s = v as f32;
        }
    ...
    fn emit_transient_aac_for_external_check() {
        ...
        for i in 0..n {
            let t = i as f64 / sr as f64;
            let mut v = 0.05 * (2.0 * core::f64::consts::PI * 220.0 * t).sin();
            // A sharp click every ~0.25 s.
            let k = i % 11025;
            if k < 700 {
                let env = (1.0 - k as f64 / 700.0).max(0.0);
                v += 0.8 * env * (2.0 * core::f64::consts::PI * 3500.0 * k as f64 / sr as f64).sin();
            }
            interleaved.push(v as f32);
        }
        let mut enc = AacEncoder::default();
    ...
    fn emit_stereo_aac_for_external_check() {
        ...
        for i in 0..n {
            let t = i as f64 / sr as f64;
            let bass = 0.4 * (2.0 * core::f64::consts::PI * 300.0 * t).sin(); // shared → M/S
            let l = bass + 0.2 * (2.0 * core::f64::consts::PI * 1200.0 * t).sin();
            let r = bass + 0.2 * (2.0 * core::f64::consts::PI * 1900.0 * t).sin();
    ...
    fn default_config_is_the_sine_arm() {
        let pcm: Vec<f32> = (0..8 * FRAME_LEN)
            .map(|i| {
                ...
                0.4 * (2.0 * core::f32::consts::PI * 700.0 * t).sin()
            })
            .collect();
    ...
    fn every_arm_round_trips() {
        ...
        let pcm: Vec<f32> = (0..n)
            .map(|i| {
                ...
                0.3 * (2.0 * core::f32::consts::PI * 440.0 * t).sin()
                    + 0.2 * (2.0 * core::f32::consts::PI * 2600.0 * t).sin()
            })
            .collect();
        ...
        let pcm: Vec<f32> = (0..n)
            .map(|i| {
                if (i / (4 * FRAME_LEN)) % 2 == 0 {
                    let t = i as f32 / sr as f32;
                    0.4 * (2.0 * core::f32::consts::PI * 900.0 * t).sin()
                } else {
    ...
    fn distinct_channels(nch: usize, frames: usize, sr: u32) -> Vec<f32> {
        ...
        for i in 0..n {
            ...
            for c in 0..nch {
                ...
                out.push(0.35 * (2.0 * core::f32::consts::PI * f * t).sin());
            }
        }
        out
    }
...
mod rung_a6 {
    ...
    fn a6_declines_on_tonal_content() {
        ...
        let pcm: Vec<f32> = (0..10 * FRAME_LEN)
            .map(|i| {
                let t = i as f32 / sr as f32;
                0.4 * (2.0 * core::f32::consts::PI * 6000.0 * t).sin()
            })
            .collect();
        let off = encode(&pcm, sr, AacEncoderConfig { bitrate_bps: 96_000, ..Default::default() });
        let on = encode(&pcm, sr, AacEncoderConfig { bitrate_bps: 96_000, pns: true, ..Default::default() });
...
mod rung_a7 {
    ...
    fn correlated_stereo(sr: u32, frames: usize, gain: f32) -> Vec<f32> {
        ...
        for i in 0..n {
            let t = i as f32 / sr as f32;
            let low = 0.3 * (2.0 * core::f32::consts::PI * 300.0 * t).sin();
            seed = seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223);

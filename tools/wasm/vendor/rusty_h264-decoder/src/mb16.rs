...
impl FrameDecoder {
    ...
    pub fn into_frame(self, crop_r: usize, crop_b: usize) -> YuvFrame {
        ...
        for row in 0..dh {
            y[row * dw..row * dw + dw]
                .copy_from_slice(&self.rec_y[row * self.cw..row * self.cw + dw]);
        }
...
pub(crate) mod edcstat {
    // ── nopeekOS-Patch (der EINZIGE, siehe ../VENDOR.md) ──────────────
    // Ohne `std` bringt der Prelude `Vec` nicht mit, und dieses verschachtelte
    // mod hat keinen eigenen Import. Upstream 0.16.0 baut den no_std-Arm
    // deshalb nicht. Diagnosecode: das `eprintln!` darin ist ohne `std`
    // ohnehin ein No-op.
    use alloc::vec::Vec;

...
pub fn read_desc(mem: &GuestMem, table: u64, idx: u16, queue_size: u16) -> Option<Desc> {
    if idx >= queue_size { return None; }
    let off = table + (idx as u64) * 16;
    Some(Desc {
        addr: mem.read_u64(off)?,
        len: mem.read_u32(off + 8)?,
        flags: mem.read_u16(off + 12)?,
        next: mem.read_u16(off + 14)?,
    })
}
...
pub fn avail_ring(mem: &GuestMem, avail_gpa: u64, queue_size: u16, slot: u16) -> Option<u16> {
    ...
    mem.read_u16(avail_gpa + 4 + i * 2)
}

/// Push a (head, len) pair to the used ring and bump used.idx with a
/// release-fence so the descriptor writes settle first.
pub fn used_push(mem: &GuestMem, used_gpa: u64, queue_size: u16, used_idx: &mut u16, head: u16, len: u32) {
    ...
    *used_idx = used_idx.wrapping_add(1);
    ...
    mem.write_u16(used_gpa + 2, *used_idx);
}
...
pub fn service_blk_queue(
    ...
    // avail.idx   @ +2
    let avail_idx = match mem.read_u16(avail + 2) {
        Some(v) => v,
        None => return false,
    };
    ...
    while *last_avail_idx != avail_idx {
        ...
        mem.write_u32(used_elem,     head_idx as u32);
        mem.write_u32(used_elem + 4, total_written);
...
fn service_one_request(
    ...
    let req_type = match mem.read_u32(head.addr) {
        Some(v) => v,
        None => return 0,
    };
    let sector = match mem.read_u64(head.addr + 8) {
        Some(v) => v,
        None => return 0,
    };

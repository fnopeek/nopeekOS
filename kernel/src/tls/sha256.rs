//! SHA-256 (FIPS 180-4)
//!
//! Required by TLS 1.3 for HKDF key derivation and transcript hashing.
...
impl Sha256 {
    pub fn new() -> Self {
        Sha256 { state: H_INIT, buf: [0; 64], buf_len: 0, total_len: 0 }
    }
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        if self.buf_len > 0 {
            let fill = (64 - self.buf_len).min(data.len());
